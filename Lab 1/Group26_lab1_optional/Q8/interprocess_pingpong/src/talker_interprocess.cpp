...
int main(int argc, char *argv[]) {
    ...
    auto chatter_pub = node->create_publisher<std_msgs::msg::String>("pingpong_test", custom_qos_profile);
    ...
    while (rclcpp::ok()) {
        eval_ros2("./evaluation/byte_data/data_256byte.txt", "./evaluation/publish_time/publish_time_256byte.txt",
                  chatter_pub);
        if (count == -1) {
    ...
    while (rclcpp::ok()) {
        eval_ros2("./evaluation/byte_data/data_128Kbyte.txt", "./evaluation/publish_time/publish_time_128Kbyte.txt",
                  chatter_pub);
        if (count == -1) {
            printf("end this data size evaluation \n");
            break;
